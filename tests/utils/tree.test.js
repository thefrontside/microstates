...
describe('Tree', () => {
  ...
  describe('with children', function() {
    beforeEach(function() {
      ...
      tree = new Tree({data: {num: 5}, children: {
  ...
  describe('from a microstate schema', function () {
    ...
    beforeEach(function() {
      tree = Tree.from(Person);
    });
    it('constructs the tree lazily', function() {
      expect(tree.children.parent.data.Type).toEqual(Person);
    });
  ...
  //     beforeEach(() => {
  //       callback = jest.fn();
  //       tree = map(callback, Tree.from(Number));
  //     });
  //     it.only('is not invoked until something is read. once', () => {
